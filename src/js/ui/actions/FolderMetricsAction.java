...
public class FolderMetricsAction implements IObjectActionDelegate{
		...
		try {
			window.run(false, true, new IRunnableWithProgress() {
		         public void run(IProgressMonitor monitor)
		            ...
		            IFolder selectedFolder = (IFolder)(((StructuredSelection)selection).getFirstElement());
		            Module module = new Module(selectedFolder);
					...
					try {
						...
						try{
							...
							JSModuleParser.getInstance().analyzeJavaScriptModuleFolder(module);
							monitor.worked(2);						
